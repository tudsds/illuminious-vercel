...
export default function Header() {
  ...
  const isDarkBgPage = pathname === "/" || pathname === "/startups";
  const isStartupsPage = pathname === "/startups";
  const isHomePage = pathname === "/";

  const getNavTextColor = (isActive: boolean) => {
    if (isStartupsPage) {
      return isActive ? "text-cyan-400" : "text-gray-300 hover:text-cyan-400";
    }
    if (isHomePage) {
      if (isScrolled) {
        return isActive ? "text-blue-600" : "text-gray-700 hover:text-blue-600";
      }
      return isActive ? "text-sky-400" : "text-white/90 hover:text-white";
    }
    return isActive ? "text-blue-600" : "text-gray-700 hover:text-blue-600";
  };

  const getLogoTextColor = () => {
    if (isStartupsPage) return "text-cyan-400";
      ...
      <nav className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16 lg:h-18">
          ...
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className={`text-lg lg:text-xl font-bold ${getLogoTextColor()}`}>
              illuminious
            </span>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-0.5">
            {navItems.map((item) => (
              ...
              >
                <Link
                  href={item.href}
                  className={`px-2.5 py-1.5 rounded-md text-sm font-medium transition-colors flex items-center gap-0.5 ${getNavTextColor(isActive(item.href))}`}
                >
                  {item.label}
                        ...
                        {item.columns?.map((column, colIdx) => (
                          <div key={column.title} className={colIdx > 0 ? "mt-3 pt-3 border-t border-gray-100" : ""}>
                            <h4 className={`text-[10px] font-semibold uppercase tracking-wider mb-2 ${
                              isStartupsPage ? "text-cyan-400/60" : "text-gray-400"
                            }`}>
                              {column.title}
                            </h4>
                                ...
                                <Link
                                  ...
                                  className={`flex items-center gap-2 px-2 py-1.5 rounded-md text-sm transition-colors ${
                                    isStartupsPage
                                      ...
                                      : "text-gray-700 hover:bg-sky-100/50 hover:text-blue-600"
                                  }`}
                                >
                                  {subItem.icon && (
                                    <subItem.icon className={`w-4 h-4 ${
                                      isStartupsPage ? "text-cyan-400/60" : "text-blue-600/60"
                                    }`} />
                                  )}
                                  {subItem.label}
                                </Link>
          ...
          <div className="hidden lg:flex items-center">
            <Button
              ...
              className={`${
                isStartupsPage
                  ? "bg-cyan-400 text-black hover:bg-cyan-400/80"
          ...
          {isMobileMenuOpen && (
            <motion.div
              ...
              className={`lg:hidden overflow-hidden ${
                isStartupsPage ? "bg-black" : "bg-white"
              }`}
            >
                              ...
                              {item.columns?.map((column) => (
                                <div key={column.title} className="py-1">
                                  <div className={`text-[10px] font-semibold uppercase tracking-wider px-3 mb-1 ${
                                    isStartupsPage ? "text-cyan-400/50" : "text-gray-400"
                                  }`}>
                                    {column.title}
                                  </div>
                                  {column.items.map((subItem) => (
